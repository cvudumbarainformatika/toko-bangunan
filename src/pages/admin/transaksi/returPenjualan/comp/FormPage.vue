<template>
  <div class="q-pa-sm">
    ...
    <div v-if="store.item?.detail?.length">
      ...
      <div  v-if="store.noRetur" class="text-center q-my-sm f-16">Nomor Retur Penjualan : {{ store.noRetur }}</div>
      ...
      <div v-for="(item, index) in store.item?.detail" :key="index">
        <div class="row items-center q-pa-sm q-col-gutter-sm">
          ...
          <div class="col-2">
            <app-input v-model="item.retur" label="Jumlah retur"
              @clear="()=>{item.retur=0}"
              @update:model-value="(val)=>{
                const _removedZeros = isNaN(parseInt(val))?0:parseInt(val)
                const sudahRetur=store.item?.detail_retur.filter(f=>f.kodebarang===item.kodebarang && f.status !== '')?.reduce((acc,cur)=>acc+cur.jumlah,0)
...
function selesai() {
  store.loading=true
  store.selesai({no_retur:store.noRetur})
  .then((data) => {
    store.loading=false
    emits('selesai',data)
